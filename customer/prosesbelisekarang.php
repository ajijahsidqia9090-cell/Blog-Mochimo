...
// 2. Logika Cek Stok & Harga (Penting untuk Varian/Non-Varian)
$stok = 0;
$harga_satuan = $p['harga']; // Default harga
$pakaiVarian = false;

// Cek apakah ada kiriman data varian
if(!empty($ukuran) || !empty($warna)){
    ...
    if($varian){
        ...
        $harga_satuan = $varian['harga_varian']; // Gunakan harga varian jika ada
        $pakaiVarian = true;
    } else {
...
// 5. Insert Pesanan
mysqli_query($conn, "INSERT INTO pesanan (id_user, tanggal_pesan, total, total_bayar, status, alamat_pengiriman, metode_pembayaran) VALUES ('$id_user', NOW(), '$total', '$total', '$metode', '$alamat', '$metode')");
$id_pesanan = mysqli_insert_id($conn);

// 6. Insert Detail Pesanan
mysqli_query($conn, "INSERT INTO detail_pesanan (id_pesanan, id_produk, ukuran, warna, qty, harga, subtotal) VALUES ('$id_pesanan', '$id_produk', '$ukuran', '$warna', '$qty', '$harga_satuan', '$total')");

// 7. Update Stok
if($pakaiVarian){
    mysqli_query($conn, "UPDATE varian_produk SET stok = stok - $qty WHERE id_produk='$id_produk' AND ukuran='$ukuran' AND warna='$warna'");
} else {
