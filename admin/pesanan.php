...
/*
|--------------------------------------------------------------------------
| UPDATE STATUS & AUTO-INSERT TRACKING
|--------------------------------------------------------------------------
*/
if(isset($_GET['status']) && isset($_GET['id'])){
    ...
    $status_db = ucfirst($status); // Mengubah status jadi: Diproses, Dikirim, Selesai

    // 1. Update status di tabel pesanan
    mysqli_query($conn, "UPDATE pesanan SET status='$status_db' WHERE id_pesanan='$id'");

    // 2. Logika riwayat untuk tabel tracking (agar nyambung ke pembeli)
    $ket = "";
    $lok = "Gudang Penjual";
    if($status == 'diproses'){
        $ket = "Pesanan sedang disiapkan dan dikemas.";
    } elseif($status == 'dikirim'){
        $ket = "Pesanan telah diserahkan ke kurir.";
        $lok = "Hub Transit";
    } elseif($status == 'selesai'){
        $ket = "Pesanan telah sampai di tujuan.";
        $lok = "Alamat Pembeli";
    }

    if($ket != ""){
        mysqli_query($conn, "INSERT INTO tracking (id_pesanan, lokasi, keterangan) VALUES ('$id', '$lok', '$ket')");
    }
...
| PERBAIKAN: Mengubah pr.foto menjadi pr.gambar sesuai struktur tabel produk Anda
|--------------------------------------------------------------------------
*/
            ...
            <div class="order-card">
                <div class="order-header">
                    ...
                    <div>
                        <span class="badge shopee-badge <?= $badge_class; ?>">
                            <i class="bi bi-info-circle-fill me-1"></i> <?= $data['status']; ?>
                        </span>
                    </div>
                </div>
                ...
                <div class="order-footer">
                    <div>
                        ...
                        <div class="price-text">Rp <?= number_format($total_tampil); ?></div>
                    </div>
